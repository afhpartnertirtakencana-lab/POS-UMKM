...
## Sinkronisasi Google Sheets (baru)

Aplikasi bisa terhubung ke Google Sheets untuk backup otomatis & laporan
online: isi **Sheet ID** dan **GAS Web App URL** di menu **Pengaturan →
Sinkronisasi Google Sheets**, aktifkan auto-sync (setiap 15 menit), atau
sync manual kapan saja lewat ikon 🔄 di topbar (tersedia di semua menu).

Perlu deploy sebuah Google Apps Script terlebih dahulu (gratis, langkahnya
5 menit) — panduan lengkapnya ada di **`PANDUAN_GOOGLE_SHEETS.md`**, dan
kode servernya ada di **`google-apps-script/Code.gs`**.
...
PANDUAN_GOOGLE_SHEETS.md  Panduan lengkap setup sinkronisasi Google Sheets
google-apps-script/
  Code.gs               Kode backend Google Apps Script (di-deploy ke Google Sheets Anda)
assets/
  db.js                 Struktur & penyimpanan data (localStorage) + data contoh
  ui.js                 Komponen UI: format, toast, modal, ikon, autentikasi
  sync.js               Logika sinkronisasi Google Sheets (push/pull/auto-sync)
